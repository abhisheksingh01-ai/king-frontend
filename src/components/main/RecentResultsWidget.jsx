...
const RecentResultsWidget = () => {
  ...
  // Date String for Header Display
  ...
  useEffect(() => {
    ...
    const fetchLiveResults = async () => {
      try {
        ...
        // 🔥 FAST: Calls the Recent (Top 3) API
    ...
    fetchLiveResults();
    const interval = setInterval(fetchLiveResults, 20000); 
  ...
  const processLiveFeedFast = (data) => {
    ...
    // The API returns [{ date: "...", games: { "GALI": {result: "XX", timestamp: ...}, ... } }]
    // Since the server already filtered for Top 3, we just need to convert the object to an array.
    const todayData = data[0]; 
    if (!todayData || !todayData.games) return;

    const gamesObj = todayData.games;
    const declaredGames = Object.entries(gamesObj).map(([name, val]) => ({
      name: name,
      result: val.result,
      timestamp: val.timestamp
    }));

    // Optional: Sort again to be 100% sure the UI order is perfect (descending time)
  ...
  const renderEmpty = () => (
    <div className="mt-4 p-8 bg-white/5 rounded-2xl border border-dashed border-white/20 text-center backdrop-blur-sm">
      ...
      <p className="text-gray-300 text-sm font-medium tracking-wide">Waiting for first result...</p>
    </div>
  );
  ...
  return (
    <div className="w-full max-w-md mx-auto mt-6 bg-linear-to-b from-slate-900 to-black p-5 rounded-3xl shadow-2xl border border-slate-800 relative overflow-hidden">
      ...
      <div className="relative z-10 flex justify-between items-center mb-6 px-1">
        <div>
          <h2 className="text-white text-xl font-black tracking-wider italic">FAST RESULT</h2>
          <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mt-1">{todayStr}</p>
        </div>
        ...
        {loading ? renderLoading() : (latestResults.length === 0 ? renderEmpty() : (
          <div className="flex flex-col gap-4">
            {latestResults.map((game, index) => {
              ...
              return (
                <div key={game.name} className={`relative rounded-2xl border transition-all duration-300 group ${isLatest ? 'bg-linear-to-r from-slate-800 to-slate-900 border-yellow-500/50 shadow-[0_4px_20px_-5px_rgba(234,179,8,0.3)] scale-100' : 'bg-white/5 border-white/10 scale-[0.98] hover:bg-white/10'}`}>
                  <div className={`flex items-center justify-between ${isLatest ? 'p-5' : 'p-4'}`}>
                    <div className="flex flex-col gap-1">
                      {isLatest && <span className="text-[9px] font-bold text-yellow-400 uppercase tracking-widest animate-pulse">Just Announced</span>}
                      <span className={`font-black uppercase tracking-tight text-white ${isLatest ? 'text-2xl' : 'text-lg text-slate-300'}`}>{game.name}</span>
                    </div>
                    <div className={`relative flex items-center justify-center rounded-full font-black font-mono shadow-inner ${isLatest ? 'w-16 h-16 text-4xl text-black bg-linear-to-br from-yellow-300 via-yellow-400 to-yellow-600 shadow-[inset_-2px_-2px_6px_rgba(0,0,0,0.2),2px_2px_6px_rgba(0,0,0,0.3)]' : 'w-12 h-12 text-xl text-white bg-gradient-to-br from-slate-600 to-slate-800 border border-slate-500 shadow-[inset_-2px_-2px_4px_rgba(0,0,0,0.3)]'}`}>
